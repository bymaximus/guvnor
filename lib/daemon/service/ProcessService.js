...
ProcessService.prototype.startProcess = function (script, options, callback) {
  ...
  if (processInfo && (!options.name || options.name === processInfo.name)) {
...
ProcessService.prototype._setupProcessCallbacks = function (processInfo, prefix) {
  ...
  processInfo.process.on(prefix + ':stopping', function () {
    this._logger.debug(prefix, processInfo.id, 'is stopping')
...
ProcessService.prototype._handleProcessStarted = function (processInfo, prefix, socket) {
  ...
  this._managedProcessFactory.create([{
    socket: socket
  }], function (error, proc) {
    if (error) {
    ...
    proc.connect(function (error, remote) {
      if (error) {
        this._logger.error('Could not create connection to remote process', error.stack)
...
ProcessService.prototype._handleProcessExit = function (processInfo, prefix, code, signal) {
  ...
  // If the code === null then we exited with a signal.  If the process was struggling to start
  // the user may have sent it SIGTERM via `guv stop` in which case we marked it 'stopping' above
  // so don't restart.  If the process has restarted itself (via ProcessRPC) it's status will be
  // 'restarting' so honour that
  if ((code !== 0 && processInfo.status !== 'stopping') || processInfo.status === 'restarting') {
...
ProcessService.prototype._restartProcess = function (processInfo, prefix) {
  ...
  if (processInfo.restarts < processInfo.restartRetries) {
    this._logger.debug('Restarting process %s %d x %d', processInfo.name, processInfo.restarts, processInfo.restartRetries)
